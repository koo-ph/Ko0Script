...
--#region WORKER_VARS
local BreakableWorker
local KA_Worker
local TPF_Worker
local ASA_Worker
local ACO_Worker
local OpenChestWorker
local OpenRingsWorker
--#endregion

--#region ============================================================== TELEPORT CONTROLLER ==============================================================
...
local function Damage(target)
	...
	if not onHit or not target:FindFirstChild("Humanoid") then
		return
	end

	onHit:FireServer(target.Humanoid, 9999, {}, 0)
	if not CollectionService:HasTag(target, "breakable") then
		return
	end
	local kb = Remotes:FindFirstChild("kb")
	if not kb or not target:FindFirstChild("Primary") then
		return
	end
	kb:FireServer(target.Primary, Vector3.new(0, 0, 0), 0.1)
end
...
TargetHandlers["BookHand"] = function(target)
	...
	if hadEntrance and health and health.Value > 0 and alive and alive.Value and target_health.Value > 0 then
		Damage(target)
	end
end
...
local function IsEnemyInZone(enemy, zone)
	...
	return math.abs(rel.X) <= half.X + EPS and math.abs(rel.Y) <= half.Y + EPS and math.abs(rel.Z) <= half.Z + EPS
end
...
local function AutoFarmTween(speed)
	...
	if not TeleportController:Acquire("FarmTP", 2) then
		return
	end
	...
	if nextZone.Parent:FindFirstChild("Entrance") then
		table.insert(targets, nextZone.Parent.Entrance.Position)
	end

	if nextZone.Parent:FindFirstChild("FLOOR") then
		table.insert(targets, nextZone.Parent.FLOOR.Position)
	end

	if nextZone.Parent:FindFirstChild("Exit") then
		table.insert(targets, nextZone.Parent.Exit.Position)
	end

	-- fallback: if neither exists, use Tp like before
	if #targets == 0 and nextZone.Parent:FindFirstChild("Tp") then
		table.insert(targets, nextZone.Parent.Tp.Position)
	end
...
local function AutoReplayWorld()
	...
	if not vote.Value or not gameEndVote then
		return
	end
...
highlight.FillTransparency = 0.95 -- Semi-transparent fill
...
local function RemoveLoadingScreen()
	...
	local gameui = plrgui:FindFirstChild("gameUI")
	if not gameui then
		return
	end
	local loading = gameui:FindFirstChild("Loading")
	if not loading then
		return
	end
...
UIGradient.Color = ColorSequence.new({
	ColorSequenceKeypoint.new(0.00, Color3.fromRGB(160, 90, 255)),
	ColorSequenceKeypoint.new(1.00, Color3.fromRGB(35, 25, 60)),
})
...
UIGradient_2.Color = ColorSequence.new({
	ColorSequenceKeypoint.new(0.00, Color3.fromRGB(110, 80, 255)),
	ColorSequenceKeypoint.new(1.00, Color3.fromRGB(30, 25, 55)),
})
...
UIGradient_3.Color = ColorSequence.new({
	ColorSequenceKeypoint.new(0.00, Color3.fromRGB(90, 60, 200)),
...
UIGradient_4.Color = ColorSequence.new({
	...
	ColorSequenceKeypoint.new(1.00, Color3.fromRGB(150, 10, 10)),
})
...
local function UpdatePerformance(dt)
	if not dt or dt <= 0 or dt > 1 then
		return -- ignore garbage frame
	end
...
local function MakeDraggable(frame: GuiObject, handle: GuiObject?)
	...
	local dragStart
	local startPos
	worker:Start(handle.InputBegan:Connect(function(input)
		if
	...
	worker:Start(UserInputService.InputChanged:Connect(function(input)
...
local function SaveFramePositions()
	local hp = HPFrame.Position
	local fps = PerfFrame.Position

	return string.format(
		"HP:%f,%d,%f,%d|FPS:%f,%d,%f,%d",
		hp.X.Scale, hp.X.Offset, hp.Y.Scale, hp.Y.Offset,
		fps.X.Scale, fps.X.Offset, fps.Y.Scale, fps.Y.Offset
	)
end
local function LoadFramePositions(data)
	local hpData, perData = data:match("HP:(.-)|FPS:(.+)")
	if not hpData or not perData then return end
	...
	HPFrame.Position = parseUDim2(hpData)
	PerfFrame.Position = parseUDim2(perData)
end

MakeDraggable(HPFrame)
MakeDraggable(PerfFrame)
--#endregion
...
local function AutoSelectAbility_v2()
	if type(UpgradeUIEnv) == "table" and #UpgradeUIEnv > 0 then
		return
	end
	local plrUpgrade = Remotes:FindFirstChild("plrUpgrade")
	if not plrUpgrade then
		return
	end
	local pg = LocalPlayer:FindFirstChild("PlayerGui")
	if not pg then
	...
	ui.Visible = false
	local banners =
		{ ui.LeftBanner.BannerCover.Banner, ui.MiddleBanner.BannerCover.Banner, ui.RightBanner.BannerCover.Banner }
	local priorities = GetPriorities() -- ability key -> slider value
...
local function AutoClaimOminous(tpf_toggle)
	...
	if nearest and tpf_toggle then
		cleanup()
		return
	end
	local character = LocalPlayer.Character
...
return function(Window, Library)
	...
	Main_Combat:AddToggle("KA_Toggle", {
		...
		Callback = function(Value)
			...
			KA_Worker = worker:StartThread(function(thread_state)
				while thread_state() and Toggles.KA_Toggle.Value do
	...
	Main_Combat:AddToggle("IB_Toggle", {
		Text = "Breakables Aura",
		Callback = function(state)
			if state then
				BreakableWorker = worker:StartThread(function(thread_state)
					while thread_state() do
						for breakable in pairs(breakables) do
							BreakAura(breakable)
						end
						task.wait(Options.KA_Speed.Value)
					end
				...
				if BreakableWorker then
					worker:Stop(BreakableWorker)
					BreakableWorker = nil
				end
			end
		end,
	...
	Main_Combat:AddToggle("TPF_Toggle", {
		...
		Callback = function(Value)
			...
			TPF_Worker = worker:StartThread(function(thread_state)
				while thread_state() and Toggles.TPF_Toggle.Value do
	...
	Main_Visual:AddInput("FP_Input", {
		Text = "Frame Positions",
		Default = SaveFramePositions(),
		ClearTextOnFocus = false,
		Tooltip = "Frames are draggable too :)",
        Callback = function(value)
        end 
	})
    LoadFramePositions(Options.FP_Input.Value)
    Options.FP_Input:OnChanged(function()
        LoadFramePositions(Options.FP_Input.Value)
    end)
	Options.HTO_ColorPicker:SetValueRGB(Options.HTO_ColorPicker.Value)
	Main_Visual:AddToggle("RLS_Toggle", {
		Text = "Skip Loading Screen",
		Tooltip = "If you got stuck lol",
	})
	--#endregion
	...
	Main_Utility:AddToggle("ASA_Toggle", {
		...
		Callback = function(Value)
			...
			ASA_Worker = worker:StartThread(function(thread_state)
				while thread_state() and Toggles.ASA_Toggle.Value do
					AutoSelectAbility()
	...
	Main_Utility:AddButton("Open All Chest", function()
		-- guard: prevent multiple spawns (keeps your intent)
		if OpenChestWorker then
			return
		end
		...
		OpenChestWorker = worker:StartThread(function(thread_state)
			while true and thread_state() do
				local hasLoot = false
				...
				if not hasLoot then
					-- release guard when finished
					if OpenChestWorker then
						worker:Stop(OpenChestWorker)
						OpenChestWorker = nil
					end
					break
				end
	...
	Main_Utility:AddButton("Open All Rings", function()
		-- 🔒 HARD LOCK (keeps your intent)
		if OpenRingsWorker then
			return
		end
		...
		OpenRingsWorker = worker:StartThread(function(thread_state)
			-- Open normal rings
			while true and thread_state() do
	...
	Main_Utility:AddToggle("ACO_Toggle", {
		...
		Callback = function(Value)
			...
			ACO_Worker = worker:StartThread(function(thread_state)
				while thread_state() and Toggles.ACO_Toggle.Value do
	...
	worker:Start(RunService.Heartbeat:Connect(function(delta_time)
		...
		if Toggles.RLS_Toggle.Value then
			RemoveLoadingScreen()
		end
	end))
	...
	worker:Start(CollectionService:GetInstanceAddedSignal("breakable"):Connect(function(b)
		breakables[b] = true
	end))
	worker:Start(CollectionService:GetInstanceRemovedSignal("breakable"):Connect(function(b)
		breakables[b] = nil
	end))
    worker:Start(HPFrame:GetPropertyChangedSignal("Position"):Connect(function()
        Options.FP_Input:SetValue(SaveFramePositions())
    end))
    worker:Start(PerfFrame:GetPropertyChangedSignal("Position"):Connect(function()
        Options.FP_Input:SetValue(SaveFramePositions())
    end))
	--#endregion
